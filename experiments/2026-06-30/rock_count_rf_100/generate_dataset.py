#!/usr/bin/env python3
"""Scaled-up (N=100) version of ../rock_count_rf/generate_dataset.py.

Same geometry/method (single ballast layer, rock=6.1/matrix=3, 420 MHz,
rock_packing_algorithm="circlify", rock_shape="circle" for exact rock-count
ground truth via `#cylinder` counting). The N=36 feasibility check found NO
robust signal (R^2 <= 0 for every model). This scales N up to see whether a
real-but-subtle effect was just undetectable at N=36 relative to 820
candidate features, per the recommendation in rock_count_rf/README.md.

Sweep: target_fill in {0.3, 0.45, 0.6, 0.75, 0.9} x 20 seeds each = 100 samples.
"""
import subprocess
import sys
import time
...
TOML_TEMPLATE = """# rock-count RF dataset (N=100): target_fill={fill:.2f}, seed={seed}
[job]
mode = "layers"
render = false
...
seed = {seed}
title = "rock-count dataset N100: fill={fill:.2f} seed={seed}"
...
def main():
    ...
    seeds = list(range(1, 21))  # 20 seeds per fill
    ...
    print("\n" + "=" * 60)
    print("DATASET GENERATION SUMMARY (N=100)")
    print("=" * 60)
