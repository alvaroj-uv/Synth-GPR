#!/usr/bin/env python3
"""
Extract DZT sample and compare with 10layer_eps_sweep.out.
Visualization with correlation metrics.
"""
...
def read_dzt_sample(dzt_path: Path, trace_idx: int = 15000, sample_range: tuple = None):
    """Extract and return DZT sample with optional range."""
    ...
    if sample_range:
        signal = signal[sample_range[0]:sample_range[1]]
...
def main():
    print("\n" + "="*90)
    print("DZT SAMPLE vs 10-LAYER EPSILON SWEEP COMPARISON")
    print("="*90 + "\n")

    # Load synthetic 10-layer
    out_path = Path("10layer_eps_sweep.out")
    syn_sig, syn_dt = read_synthetic_out(out_path)
    syn_t = np.arange(len(syn_sig)) * syn_dt

    print(f"[SYNTHETIC - 10LAYER]")
    ...
    ax1.plot(syn_t, syn_sig_flip, color="#00d9ff", lw=1, alpha=0.8, label="Synthetic (polarity corrected)")
    ...
    ax1.set_title("10-Layer Synthetic (Full Trace)", fontsize=11, color="#00d9ff", fontweight='bold')
    ...
    ax2.plot(real_t, real_sig, color="#ff6b35", lw=1, alpha=0.8, label="Real DZT Sample")
    ...
    ax2.set_title("Real DZT Trace #15000 (Full Trace)", fontsize=11, color="#ff6b35", fontweight='bold')
    ...
    fig.suptitle(
        "10-Layer Epsilon Sweep (Synthetic) vs Real DZT Sample\n" +
        "Trace #15000, Polarity Corrected, Interpolated to Common Grid",
        color="#c8d0e0", fontsize=13, fontweight='bold', y=0.995
    )

    png_path = Path("output_test") / "compare_dzt_10layer.png"
