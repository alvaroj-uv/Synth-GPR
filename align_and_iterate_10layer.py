#!/usr/bin/env python3
"""
Align 10layer_eps_sweep_50ns.out first wave with DZT real data.
Iterate to find best alignment and time-shift.
"""
...
def find_dw_peak(signal: np.ndarray, dt_ns: float, search_ns: float = 20):
    """Find first significant peak (DW)."""
...
def time_shift_signal(signal: np.ndarray, shift_ns: float, dt_ns: float):
    """Shift signal in time by inserting/removing samples at start."""
    shift_samples = int(round(shift_ns / dt_ns))

    if shift_samples > 0:
        # Shift right (delay) - prepend zeros
        shifted = np.concatenate([np.zeros(shift_samples), signal[:-shift_samples]])
    elif shift_samples < 0:
        # Shift left (advance) - remove samples from start
...
def compute_correlation(syn_sig: np.ndarray, syn_dt: float,
                       real_sig: np.ndarray, real_dt: float):
    """Compute correlation after interpolation to common grid."""
    ...
    return corr, t_common, syn_norm, real_norm


def main():
    print("\n" + "="*90)
    print("ALIGN 10-LAYER FIRST WAVE AND ITERATE")
    print("="*90 + "\n")

    # Load signals
    syn_path = Path("10layer_eps_sweep_50ns.out")
    ...
    print(f"  Time diff: {real_peak_t - syn_peak_t:.4f} ns\n")

    # Iterate with different time shifts
    print("="*90)
    print("ITERATION: Testing time shifts to align peaks")
    print("="*90 + "\n")

    shift_range = np.arange(0, real_peak_t + 1.0, 0.5)  # 0 to 6+ ns in 0.5ns steps
    results = []

    print(f"{'Shift (ns)':<15} {'Peak offset (ns)':<18} {'Correlation':<15}")
    print("-" * 50)

    for shift_ns in shift_range:
        shifted_sig, _ = time_shift_signal(syn_sig, shift_ns, syn_dt)

        # Compute new peak position
        shifted_peak_idx, shifted_peak_t = find_dw_peak(shifted_sig, syn_dt)

        # Compute correlation
        corr, _, _, _ = compute_correlation(shifted_sig, syn_dt, real_sig, real_dt)

        peak_offset = abs(shifted_peak_t - real_peak_t)
        results.append((shift_ns, peak_offset, corr, shifted_sig))

        marker = " <-- BEST" if len(results) > 0 and corr == max(r[2] for r in results) else ""
        print(f"{shift_ns:<15.1f} {peak_offset:<18.4f} {corr:+.6f}{marker}")

    # Find best correlation
    best_idx = np.argmax([r[2] for r in results])
    best_shift, best_offset, best_corr, best_sig = results[best_idx]

    print(f"\n{'='*90}")
    print("OPTIMAL ALIGNMENT")
    print(f"{'='*90}\n")

    print(f"Best time shift: {best_shift:.1f} ns")
    print(f"Peak alignment offset: {best_offset:.4f} ns")
    print(f"Correlation: {best_corr:+.6f}\n")

    # Generate comparison visualization
    corr, t_common, syn_norm, real_norm = compute_correlation(best_sig, syn_dt,
                                                               real_sig, real_dt)
    ...
    # Time arrays
    syn_t = np.arange(len(best_sig)) * syn_dt
    real_t = np.arange(len(real_sig)) * real_dt

    # Row 1: Aligned signals
    ax1 = fig.add_subplot(gs[0, 0])
    ax1.set_facecolor("#1a1e2b")
    ax1.plot(syn_t, best_sig, color="#00d9ff", lw=1, alpha=0.8, label="Synthetic (shifted)")
    syn_peak_idx_best, syn_peak_t_best = find_dw_peak(best_sig, syn_dt)
    ax1.scatter([syn_peak_t_best], [best_sig[syn_peak_idx_best]], color='#ffff00', s=150,
    ...
    ax1.set_title(f"Synthetic (shifted +{best_shift:.1f}ns)", fontsize=11, color="#00d9ff", fontweight='bold')
    ...
    ax2.plot(real_t, real_sig, color="#ff6b35", lw=1, alpha=0.8, label="Real DZT")
    ...
    ax2.set_title("Real DZT Sample", fontsize=11, color="#ff6b35", fontweight='bold')
    ...
    mask_syn = syn_t <= 30
    ax3.plot(syn_t[mask_syn], best_sig[mask_syn], color="#00d9ff", lw=1.5, alpha=0.8)
    ax3.axhline(0, color='#2a2f42', lw=0.8, alpha=0.5)
    ax3.set_ylabel("Amplitude (V/m)", fontsize=10, color="#c8d0e0", fontweight='bold')
    ax3.set_title("Synthetic (0-30ns)", fontsize=11, color="#00d9ff", fontweight='bold')
    ...
    ax4.set_title("Real (0-30ns)", fontsize=11, color="#ff6b35", fontweight='bold')
    ...
    fig.suptitle(
        f"10-Layer Aligned with DZT (time shift +{best_shift:.1f}ns)\n" +
        f"Best correlation: {corr:+.4f}, Peak offset: {best_offset:.4f}ns",
        color="#c8d0e0", fontsize=13, fontweight='bold', y=0.995
    )

    png_path = Path("output_test") / "align_10layer_iterated.png"
