#!/usr/bin/env python3
"""
Compare thin sand interface layer vs validated single-layer model.
"""
...
with h5py.File("test_sand_interface.out", 'r') as f:
    interface_sig = -f['rxs/rx1/Ez'][()]
    interface_dt = f.attrs.get('dt', 0.0) * 1e9
...
interface_sig = interface_sig / np.max(np.abs(interface_sig))
real_sig = real_sig / np.max(np.abs(real_sig))

# Time arrays
validated_t = np.arange(len(validated_sig)) * validated_dt + 4.0
interface_t = np.arange(len(interface_sig)) * interface_dt + 4.0
real_t = np.arange(len(real_sig)) * real_dt

print("\n" + "="*80)
print("SAND INTERFACE LAYER COMPARISON")
print("="*80 + "\n")

# Correlations
common_dt = real_dt
t_max = min(validated_t[-1], interface_t[-1], real_t[-1])
...
f_interface = interp1d(interface_t, interface_sig, kind='cubic', bounds_error=False, fill_value=0)
f_real = interp1d(real_t, real_sig, kind='cubic', bounds_error=False, fill_value=0)

validated_i = f_validated(tc)
interface_i = f_interface(tc)
real_i = f_real(tc)

# Coda
mask_coda = tc >= 9
r_validated_coda = np.corrcoef(validated_i[mask_coda], real_i[mask_coda])[0, 1]
r_interface_coda = np.corrcoef(interface_i[mask_coda], real_i[mask_coda])[0, 1]

print(f"VALIDATED MODEL (Single-layer, 300mm):")
print(f"  Coda (9-35ns): {r_validated_coda:+.6f}\n")

print(f"SAND INTERFACE MODEL (3cm sand + 27cm rocks):")
print(f"  Coda (9-35ns): {r_interface_coda:+.6f}\n")

improvement = r_interface_coda - r_validated_coda
...
if improvement > 0.03:
    verdict = "MEANINGFUL IMPROVEMENT - Consider for production"
elif improvement > -0.01:
    verdict = "ESSENTIALLY EQUIVALENT - Thin layer negligible effect"
else:
    verdict = "MARGINALLY WORSE - No benefit"
...
ax1.set_title("Validated (Single 300mm layer)", fontsize=12, color="#00ffff", fontweight='bold')
...
ax2.plot(interface_t[interface_t<=50], interface_sig[interface_t<=50], color="#00ff00", lw=1.5, alpha=0.8)
ax2.axvspan(9, 35, alpha=0.08, color='#ffff00')
ax2.set_ylabel("Amplitude", fontsize=11, color="#c8d0e0", fontweight='bold')
ax2.set_title("Sand Interface (3cm sand + 27cm rocks)", fontsize=12, color="#00ff00", fontweight='bold')
...
ax3.fill_between(tc[mask_coda], validated_i[mask_coda], real_i[mask_coda], alpha=0.1, color='cyan')
...
ax4.plot(tc[mask_coda], interface_i[mask_coda], color="#00ff00", lw=2.5, alpha=0.85, label="Interface")
ax4.plot(tc[mask_coda], real_i[mask_coda], color="#ff6b35", lw=2.5, alpha=0.85, label="Real")
ax4.fill_between(tc[mask_coda], interface_i[mask_coda], real_i[mask_coda], alpha=0.1, color='green')
...
status_color = "#00ff00" if improvement > 0.01 else "#ff0000"
ax4.set_title(f"Sand Interface vs Real (r={r_interface_coda:+.4f}) [{improvement:+.4f}]",
...
fig.suptitle(
    f"Thin Sand Interface Test: Does a 3cm surface layer help?\n" +
    f"Validated={r_validated_coda:+.4f} vs Interface={r_interface_coda:+.4f} (diff={improvement:+.4f}, {pct_change:+.1f}%)",
    color="#c8d0e0", fontsize=13, fontweight='bold', y=0.97
)

png_path = Path("output_test") / "compare_sand_interface.png"
