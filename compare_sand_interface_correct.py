#!/usr/bin/env python3
"""
Compare CORRECT sand interface (sand on TOP) vs validated model.
"""
...
with h5py.File("test_sand_interface_correct.out", 'r') as f:
...
print("\n" + "="*80)
print("CORRECT SAND INTERFACE COMPARISON (Sand on TOP)")
print("="*80 + "\n")
...
print(f"VALIDATED MODEL (Single 300mm):")
print(f"  Coda (9-35ns): {r_validated_coda:+.6f}\n")

print(f"SAND INTERFACE MODEL (27cm rocks + 3cm sand on top):")
...
if improvement > 0.03:
    verdict = "IMPROVEMENT - Worth considering"
elif improvement > -0.02:
    verdict = "ESSENTIALLY EQUIVALENT - Thin layer has negligible effect"
else:
    verdict = "WORSE - No benefit"
...
ax1.set_title("Validated (Single 300mm)", fontsize=12, color="#00ffff", fontweight='bold')
...
ax2.plot(interface_t[interface_t<=50], interface_sig[interface_t<=50], color="#ff9500", lw=1.5, alpha=0.8)
ax2.axvspan(9, 35, alpha=0.08, color='#ffff00')
ax2.set_ylabel("Amplitude", fontsize=11, color="#c8d0e0", fontweight='bold')
ax2.set_title("Sand on TOP (27cm rocks + 3cm sand)", fontsize=12, color="#ff9500", fontweight='bold')
...
ax4.plot(tc[mask_coda], interface_i[mask_coda], color="#ff9500", lw=2.5, alpha=0.85, label="Sand on top")
ax4.plot(tc[mask_coda], real_i[mask_coda], color="#ff6b35", lw=2.5, alpha=0.85, label="Real")
ax4.fill_between(tc[mask_coda], interface_i[mask_coda], real_i[mask_coda], alpha=0.1, color='orange')
...
ax4.set_title(f"Sand on TOP vs Real (r={r_interface_coda:+.4f}) [{improvement:+.4f}]",
...
fig.suptitle(
    f"Correct Sand Interface: Does a 3cm weathered layer on top help?\n" +
    f"Validated={r_validated_coda:+.4f} vs Sand-on-top={r_interface_coda:+.4f} (diff={improvement:+.4f}, {pct_change:+.1f}%)",
    color="#c8d0e0", fontsize=13, fontweight='bold', y=0.97
)

png_path = Path("output_test") / "compare_sand_interface_correct.png"
