...
class AnnotatedGPRMaxFileWriter:
    ...
    def _antenna_explanation(metadata: Dict[str, Any]) -> str:
        ...
        lines = [
            ...
            "## " + "=" * 78,
            "##",
            "## Antenna Type: Monostatic (Single TX/RX Antenna)",
            "##   Single antenna transmits and receives (time-separated)",
            "##   Standard for railway ballast inspection (Mbubia et al. 2026)",
            "##",
            f"## Frequency: {metadata.get('frequency_mhz', 400):.0f} MHz",
            f"##   Wavelength in free space: {3e8 / (metadata.get('frequency_mhz', 400) * 1e6):.2f} m",
            "##",
            f"## Antenna Position: ({metadata.get('tx_x', 1.124):.3f}, {metadata.get('tx_y', 1.05):.2f}, {metadata.get('tx_z', 0.00660):.5f}) m",
            f"##   TX and RX co-located at same position",
            f"##",
            f"## Configuration:",
            f"##   Transmitter (TX) and Receiver (RX) share same antenna",
            f"##   Interpretation: Industry-standard railway GPR configuration",
            "##",
