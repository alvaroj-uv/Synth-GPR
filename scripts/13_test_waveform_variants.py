#!/usr/bin/env python3
"""
Step 13: Test source waveform variants at optimized frequency (420 MHz).
Compare Ricker, Gaussian, Gaussian Derivative.
"""
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Test source waveform variants at 420 MHz")
    ap.add_argument("real_dzt", type=Path, help="Real .DZT file")
    ap.add_argument("--waveforms", nargs="+", type=str,
                   default=["ricker", "gaussian", "gaussianderiv"],
                   help="Waveforms to test")
    ap.add_argument("--trace", type=int, default=1000)
    ap.add_argument("--output-dir", type=Path, default=Path("output_test/waveform_opt"))
    args = ap.parse_args()
    ...
    for waveform in args.waveforms:
        config_name = f"waveform_{waveform}"
        print(f"[TEST] {waveform}...", end=" ", flush=True)

        # Find TOML file
        toml_file = Path(f"examples/freespace_420mhz_{waveform}.toml")
    ...
    # Summary
    print(f"\n{'='*70}")
    print("WAVEFORM VARIANT RESULTS (420 MHz, Bistatic 30mm)")
    print(f"{'='*70}\n")
    ...
    for i, res in enumerate(results_sorted, 1):
        print(f"{i}. {res['waveform']:15s}: {res['correlation']:+.6f}")

    best = results_sorted[0]
    print(f"\n[BEST] {best['waveform']} - Correlation {best['correlation']:+.6f}")

    # Save results
    with open(output_dir / "waveform_results.json", 'w') as f:
        json.dump(results_sorted, f, indent=2)

    print(f"\n[SAVE] -> {output_dir / 'waveform_results.json'}")
