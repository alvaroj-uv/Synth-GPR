#!/usr/bin/env python3
"""
Epsilon (permittivity) sweep for material optimization.
Test epsilon values [2.5 to 5.0] to find best synthetic-real match.
Uses direct wave alignment for fair comparison.
"""
...
def create_ballast_toml(eps: float, output_path: Path) -> None:
    ...
    content = f"""# Ballast layer with eps={eps} (sweep test)
[job]
render = false
...
def main():
    ...
    epsilon_values = np.arange(2.5, 5.1, 0.1)
    epsilon_values = np.round(epsilon_values, 1)

    work_dir = Path("output_test/epsilon_sweep")
    work_dir.mkdir(parents=True, exist_ok=True)

    print(f"\n{'='*70}")
    print("EPSILON SWEEP: Material Optimization")
    print(f"{'='*70}")
    ...
    scatter = ax.scatter(eps_vals, corr_vals, s=150, c=corr_vals, cmap='RdYlGn',
                        edgecolors='#c8d0e0', linewidth=2, alpha=0.8,
                        vmin=min(corr_vals)-0.01, vmax=max(corr_vals)+0.01)

    # Mark best
    ax.scatter([best_eps], [best_corr], s=400, marker='*', color='gold',
    ...
    ax.plot(eps_sorted, corr_at_sorted, 'c--', alpha=0.4, linewidth=1.5, label='Trend')

    ax.axhline(0.914, color='#ff6b35', linestyle=':', linewidth=1, alpha=0.5, label='Previous ballast (3.45)')

    ax.set_xlabel("Epsilon (Material Permittivity)", fontsize=12, color="#c8d0e0", fontweight='bold')
    ax.set_ylabel("Correlation with Real Field GPR", fontsize=12, color="#c8d0e0", fontweight='bold')
    ax.set_title("Epsilon Sweep: Material Optimization (420 MHz Gaussian Ballast)",
                fontsize=13, color="#c8d0e0", fontweight='bold')

    ax.grid(True, color="#2a2f42", alpha=0.3)
    ax.legend(fontsize=11, facecolor='#1a1e2b', labelcolor='#c8d0e0', edgecolor='#2a2f42')
    ...
    png_path = work_dir / "epsilon_sweep.png"
